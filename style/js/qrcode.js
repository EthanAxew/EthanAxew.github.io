...
! function() {
	...
	function r(t, e) {
		if(t.length == undefined) throw new Error(t.length + "/" + e);
	...
	t.prototype = {
		...
		for(var _ = new Array(v), C = 0, g = 0; g < i; g++)
	...
	r.prototype = {
		...
		multiply: function(t) {
			for(var e = new Array(this.getLength() + t.getLength() - 1), o = 0; o < this.getLength(); o++)
				for(var i = 0; i < t.getLength(); i++) e[o + i] ^= g.gexp(g.glog(this.get(o)) + g.glog(t.get(i)));
			return new r(e, 0)
		},
		mod: function(t) {
		...
		if(r == undefined) throw new Error("bad rs block @ typeNumber:" + t + "/errorCorrectLevel:" + e);
